...
import React, { useState } from "react";
import "./styles.css";
import { applyJob } from "../api/auth";  // 👈 new API call
...
const ApplyJobs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      email: formData.email,
      password: formData.password,
      role: role.toLowerCase(),  // doctor, nurse, etc.
      profile: {
        fullName: formData.fullName,
        phone: formData.phone,
        gender: formData.gender,
    ...
    const res = await applyJob(payload);
    if (res.user) {
      alert(`Registered as ${role}. Now upload your CV.`);
      window.open("https://docs.google.com/forms/d/166EXeFn0y7R4Yunn_7XWc3JmliFTXcZaDOkGUcbDXG4/edit", "_blank");
    } else {
      alert(res.error || "Failed");
    }
  };
        ...
        <div className="content-box">
          ...
          {role && (
            <form onSubmit={handleSubmit} style={{ textAlign: "left" }}>
              ...
              <button className="btn btn-primary" type="submit">Submit & Upload CV</button>
            </form>
          )}
        </div>
      </main>
