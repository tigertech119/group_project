...
// ✅ added: rate limiting for sensitive routes
const rateLimit = require("express-rate-limit");
const authLimiter = rateLimit({
  windowMs: 10 * 60 * 1000, // 10 minutes
  max: 30,                   // 30 attempts / 10 min per IP per route
...
function setAuthCookie(res, uid, userType) {
  // ✅ Token expires in 1h (safer than multi-day)
  const token = jwt.sign({ uid, userType }, process.env.JWT_SECRET, { expiresIn: "1h" });

  res.cookie("token", token, {
    httpOnly: true,
    sameSite: process.env.NODE_ENV === "production" ? "none" : "lax",
    secure: process.env.NODE_ENV === "production",
    maxAge: 60 * 60 * 1000 // ✅ 1h expiry in ms
  });
}
...
// REGISTER (Patients)
// ==========================
router.post("/register", authLimiter, async (req, res) => {  // ✅ added limiter
  try {
...
// ==========================
router.post("/apply-job", authLimiter, async (req, res) => { // ✅ added limiter
  try {
...
// ==========================
router.post("/verify-email", authLimiter, async (req, res) => { // ✅ added limiter
  try {
...
// LOGIN
// ==========================
router.post("/login", authLimiter, async (req, res) => { // ✅ added limiter
  try {
...
// FORGOT PASSWORD
// ==========================
router.post("/forgot-password", authLimiter, async (req, res) => { // ✅ added limiter
  try {
...
// RESET PASSWORD
// ==========================
router.post("/reset-password", authLimiter, async (req, res) => { // ✅ added limiter
  try {
...
// ==========================
function setAuthCookie(res, uid, userType) {
  // ✅ Token now expires in 1h instead of 7d for safety
  const token = jwt.sign({ uid, userType }, process.env.JWT_SECRET, { expiresIn: "1h" });
